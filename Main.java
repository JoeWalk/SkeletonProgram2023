...
public class Main {
    ...
    String getInstruction() {
        ...
        String instruction;
        System.out.print("\n> ");
        instruction = input.nextLine().toLowerCase();
        return instruction;
    }
    ...
    boolean go(Character you, String direction, Place currentPlace) {
        ...
        if (!moved) {
            System.out.println("You are not able to go in that direction.");
        }
        return moved;
    }
        ...
        if (itemToExamine.equals("inventory")) {
            ...
            if (indexOfItem != -1) {
                if (items.get(indexOfItem).name.equals(itemToExamine) && (items.get(indexOfItem).location == INVENTORY || items.get(indexOfItem).location == currentLocation)) {
                    System.out.println(items.get(indexOfItem).description);
            ...
            while (count < characters.size()) {
                if (characters.get(count).name.equals(itemToExamine) && characters.get(count).currentLocation == currentLocation) {
                    System.out.println(characters.get(count).description);
                    return;
                }
                count++;
            }
            System.out.println("You cannot find " + itemToExamine + " to look at.");
        }
    }
    ...
    int rollDie(String lower, String upper) {
        ...
        if (isNumeric(lower)){
            ...
            while (lowerLimitValue < 1 || lowerLimitValue > 6) {                
                ...
                lowerLimitValue = Integer.parseInt(input.nextLine());
            }            
        }
        ...
        if (isNumeric(upper)) {
            ...
            while (upperLimitValue < lowerLimitValue || upperLimitValue > 6) {                
                ...
                upperLimitValue = Integer.parseInt(input.nextLine());
            }     
        }
    ...
    void readItem(ArrayList<Item> items, String itemToRead, int currentLocation) {
        ...
        if (indexOfItem == -1) {
            System.out.println("You can't find " + itemToRead + ".");
        } else if (! items.get(indexOfItem).commands.contains("read")) {
            System.out.println("You can't read " + itemToRead + ".");
    ...
    boolean getItem(ArrayList<Item> items, String itemToGet, int currentLocation) {
        ...
        if (indexOfItem == -1) {
            System.out.println("You can't find " + itemToGet + ".");
        } else if (items.get(indexOfItem).location == INVENTORY) {
            System.out.println("You have already got that!");
        } else if (!items.get(indexOfItem).commands.contains("get")) {
            System.out.println("You can't get " + itemToGet + ".");
        ...
        if (canGet) {
            ...
            if (items.get(indexOfItem).status.contains("gettable")) {
                ...
                System.out.println("You have got that now.");
            }
        }
        return stopGame;
    }
    ...
    void takeItemFromOtherCharacter(ArrayList<Item> items, int otherCharacterID) {
        ...
        count = 1;
        System.out.print("Which item do you want to take?  They have: ");
        System.out.print(listOfNamesOfItemsInInventory.get(0));
        while (count < listOfNamesOfItemsInInventory.size() - 1) {
            System.out.print(", " + listOfNamesOfItemsInInventory.get(count));
            count++;
        }
        System.out.println(".");
        String chosenItem = input.nextLine();
        if (listOfNamesOfItemsInInventory.contains(chosenItem)) {
            System.out.println("You have that now.");
            ...
            System.out.println("They don't have that item, so you don't take anything this time.");
        }
    }
    ...
    void takeRandomItemFromPlayer(ArrayList<Item> items, int otherCharacterID) {
        ...
        System.out.println("They have taken your " + items.get(listofIndicesOfItemsInInventory.get(rno)).name + ".");
    ...
    void playDiceGame(ArrayList<Character> characters, ArrayList<Item> items, String otherCharacterName) {
        ...
        if (!diceGamePossible) {
            System.out.println("You can't play a dice game.");
        } else {
            ...
            System.out.println("You rolled a " + playerScore + ".");
            ...
            System.out.println("They rolled a " + otherCharacterScore + ".");
    ...
    void moveItem(ArrayList<Item> items, String itemToMove, int currentLocation) {
            ...
            if (items.get(indexOfItem).location == currentLocation) {
                if (items.get(indexOfItem).commands.length() >= 4) {
                    if (items.get(indexOfItem).commands.contains("move")) {
                        ...
                        System.out.println("You can't move " + itemToMove + ".");
                    }
                } else {
                    System.out.println("You can't move " + itemToMove + ".");
                }
        ...
        System.out.println("You can't find " + itemToMove + ".");
    }
    ...
    void playGame(ArrayList<Character> characters, ArrayList<Item> items, ArrayList<Place> places) {
        ...
        while (!stopGame) {
            if (moved) {
                System.out.println();
                System.out.println();
                System.out.println(places.get(characters.get(0).currentLocation - 1).description);
            ...
            switch (command)
            {
                ...
                case "drop":
                    ...
                    break;
                default:
                    System.out.println("Sorry, you don't know how to " + command + ".");
            }
        }
        input.nextLine();
    }
    ...
    public static void dropItem(ArrayList<Item> items, String item, int currentLocation) {

        ArrayList<Item> itemsInIventory = new ArrayList<Item>();
        boolean valid = false;
        for (Item thing : items) {
            if (thing.location == INVENTORY) {
                itemsInIventory.add(thing);
            }
        }
        for (int i = 0; i < itemsInIventory.size(); i++) {
            if ((itemsInIventory.get(i).name).equals(item)) {
                itemsInIventory.get(i).location = currentLocation;
                valid = true;
            }
        }
        if (valid == true) {
